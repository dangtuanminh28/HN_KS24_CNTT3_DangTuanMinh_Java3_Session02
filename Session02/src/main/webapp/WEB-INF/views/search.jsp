...
<p>Số ký tự của từ khóa: ${fn:length(keyword)}</p>
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            ...
                            <c:otherwise>
                                <fmt:formatNumber value="${event.price}" pattern="#,###" /> đ
                            </c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${event.remainingTickets == 0}">HẾT VÉ</c:when>
                            <c:when test="${event.remainingTickets < 10}">
                                Sắp hết (còn ${event.remainingTickets} vé)
                            </c:when>
                            <c:otherwise>${event.remainingTickets}</c:otherwise>
                        </c:choose>
                    </td>
                    <td align="center">
